...
- **Four prefixed Cursor commands** in `commands/`: `nimble-setup` (transport preflight / connection guidance), `nimble-extract` (single-URL read via the Extract Waterfall), `nimble-research` (Web Search Agent create-or-reuse run), `nimble-status` (poll a run and fetch its result). Each delegates to the `nimble-web-expert` skill and the shared playbook; none use `curl | sh` or print credential values. The existing unprefixed `search` command is unchanged.
- **`rules/nimble-web-evidence.mdc`** — a Cursor rule (agent-requested, not always-applied) covering evidence discipline for live web data: source URL and date on every claim, preserving Web Search Agent trust/citation metadata, the stable `nimble-agent-skills` attribution, surfacing structured failures instead of empty results, and never printing `NIMBLE_API_KEY`. This restores the root `rules/` surface removed in `89cfd2a` (deletion tracked as a gap in INNOV-587 evidence), with new content distinct from the CLI-routing rule already vendored at `skills/nimble-web-expert/rules/nimble-web-expert.mdc`.

### Changed
- **`.cursor-plugin/plugin.json` declares its full component surface.** Adds `agents` (`./agents/`), `commands` (`./commands/`), `rules` (`./rules/`), and `mcpServers` (`./mcp.json`, the portable Agent Plugins configuration at the plugin root). Cursor's current reference explicitly supports that standard schema, so the hosted MCP remains single-source instead of gaining a Cursor-only copy. Verified against the current schema at cursor.com/docs/reference/plugins rather than assumed.
