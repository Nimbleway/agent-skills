...
- **Request attribution** is now the stable integration tag `nimble-agent-skills` via `--client-source` / `CLIENT_SOURCE` (CLI). The managed MCP transport records only its own source and cannot propagate integration attribution — documented as an accepted limitation, not worked around.
